    ...
    def __init__(
        ...
        *,
        ...
        if source_config_path is not None:
            source_config = json.load(open(source_config_path, "r"))
        ...
        for data in read_file(path):
            ...
            # LNY: Source-config-based processing: use message_processor, reward_model, remote_environment from config
            if source_key and source_key in data:
                source = data[source_key]
                assert source in source_config, f'Data source "{source}" is not defined in {source_config_path}'

                input_messages = data[prompt_key]
                label = data.get(label_key, None)
                tools = data.get(tool_key, [])
                ...
                prompt = message_processor.apply_chat_template(input_messages)
                mp_name = source_config[source]["message_processor"]["class"].split(".")[-1]
                print(f"!!!!!!!!!!!!message_processor: {mp_name}")
                # with open(f"prompt_mp_{mp_name}.txt", "w") as f:
                #     f.write(f"prompt:\n{prompt}")
                ...
                # reward_model_info
                reward_model_info = deepcopy(source_config[source]["reward_model"])
            ...
            else:  # default processing: use apply_chat_template / multimodal_keys
        ...
        logger.info(f"Dataset loaded from {path}: {len(self.samples)} samples loaded (total_read={total_read})")
