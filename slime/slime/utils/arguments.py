...
def get_slime_extra_args_provider(add_custom_arguments=None):
    def add_slime_arguments(parser):
        ...
        parser.add_argument("--source-key", type=str, default=None, help="JSON dataset key for data source routing")
        parser.add_argument("--eval-source-key", type=str, default=None, help="JSON dataset key for eval data source routing")
        parser.add_argument(
            "--eval-results-save-dir",
            type=str,
            default=None,
            help="The direction to save detailed evaluation results for each evaluation dataset.",
        )
        parser.add_argument(
            "--source-config-path",
            type=str,
            default=None,
            help="Path to source config JSON with per-source reward model and remote env configs.",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Stop rollout generation once an ill-formed response is detected.",
        )
        parser.add_argument(
            "--tool-max-retry",
            type=int,
            default=5,
            help="Maximum number of retries for tool calls.",
        )
        parser.add_argument(
            "--tool-timeout",
            type=int,
            default=300,
            help="Timeout in seconds for tool calls.",
        )
        parser.add_argument(
            "--rubric-reward-ratio",
            type=float,
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="If set, reward server evaluates rubric using reasoning content (not final content). Sent in extra_reward_info to /evaluate.",
        )
        parser.add_argument(
            "--rubric-use-content",
            action="store_true",
            default=False,
            help="If set, reward server evaluates rubric using final content (not reasoning). Sent in extra_reward_info to /evaluate.",
        )
        parser.add_argument(
            "--only-eval",
            action="store_true",
            default=False,
            help="only eval, no train",
        )
...
def slime_validate_args(args):
    ...
    # NOTE: `--only-eval` reuses `debug_rollout_only` path so that placement_groups only allocates rollout GPUs (no actor GPUs needed).
    if args.only_eval:
