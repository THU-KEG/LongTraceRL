...
async def generate_and_rm(
    ...
    if isinstance(sample, list):
        ...
        if sample.reward is None:
            # LNY: inject rubric_reward_ratio for the remote RM
            if "rubric_reward_ratio" not in sample.metadata["extra_reward_info"]:
                sample.metadata["extra_reward_info"]["rubric_reward_ratio"] = args.rubric_reward_ratio if not evaluation else 0  # LNY: rubric_reward_ratio is 0 during eval
            if "rubric_use_reasoning_content" not in sample.metadata["extra_reward_info"]:
                sample.metadata["extra_reward_info"]["rubric_use_reasoning_content"] = args.rubric_use_reasoning_content
                if args.rubric_use_reasoning_content:
                    print(f"rubric_use_reasoning_content")
            if "rubric_use_content" not in sample.metadata["extra_reward_info"]:
                sample.metadata["extra_reward_info"]["rubric_use_content"] = args.rubric_use_content
                if args.rubric_use_content:
                    print(f"rubric_use_content")
            
            with trace_span(sample, "reward_model"):
                reward_result = await async_rm(args, sample)  # {"outcome_reward": 0, "rubric_reward": 0, "reward": 0}
            
            assert isinstance(reward_result, dict)
            sample.reward = reward_result["reward"]
            if "rubric_reward" in reward_result:
                sample.metadata["outcome_reward"] = reward_result["outcome_reward"]
                sample.metadata["rubric_reward"] = reward_result["rubric_reward"]
...
async def generate_and_rm_group(
    ...
    # LNY: eval datasets do not enter this logic (eval_rollout_single_dataset calls generate_and_rm instead)
    if "rubric_reward" in group[0].metadata:
        # LNY: normalize each sample's reward based on the entire group
        if args.normalize_rubric_reward:
            max_rubric_reward = max(sample.metadata["rubric_reward"] for sample in group)
            print(f"MAX_RUBRIC_REWARD: {max_rubric_reward}")

        for sample in group:
            outcome_reward = sample.metadata["outcome_reward"]
            rubric_reward = sample.metadata["rubric_reward"]
            
            # LNY: choose rubric_reward value based on whether normalization is enabled
            if args.normalize_rubric_reward and max_rubric_reward > 0:
                rubric_reward = rubric_reward / max_rubric_reward

            dataset_name = sample.metadata.get("extra_reward_info", {}).get("dataset_name", "")
            if dataset_name == "longrlvr":  # LNY: longrlvr dataset uses the paper formula for reward
                # Paper formula: r_total = r_ans + η·F_β + (1-η)·r_ans·F_β
                eta = args.rubric_reward_ratio
                r_ctx = eta * rubric_reward + (1 - eta) * outcome_reward * rubric_reward
                print(f"longrlvr r_ctx: {r_ctx}")
                sample.reward = outcome_reward + r_ctx
            elif args.rubric_only_negative:
                sample.reward = outcome_reward if outcome_reward > 0 else (args.rubric_reward_ratio * rubric_reward)
            elif args.rubric_only_positive:
                print(f"rubric_only_positive: outcome_reward={outcome_reward} rubric_reward={rubric_reward}")
                sample.reward = outcome_reward if outcome_reward == 0 else ((1 - args.rubric_reward_ratio) * outcome_reward + args.rubric_reward_ratio * rubric_reward)
            else:
            ...
            task_unfinished = sample.metadata["task_unfinished"] and not ill_formed
            print(f"Final Reward: {sample.reward} | Ill Formed: {ill_formed} | Task Unfinished: {task_unfinished} | Status: {sample.status}")
...
async def generate_rollout_async(
    ...
    while len(data) < target_data_size:
        ...
        for task in done:
            ...
            if do_print:
                ...
                logger.info(
                    f"First rollout sample: {[str(sample.prompt)[:200] + '...' + str(sample.prompt)[-200:] + sample.response]}, label: {str(sample.label)[:100]}, reward: {sample.reward}",
                )
                do_print = False
    ...
    logger.info(
        f"Finish rollout: {[str(sample.prompt)[:200] + '...' + str(sample.prompt)[-200:] + sample.response]}, label: {str(sample.label)[:100]}, reward: {sample.reward}",
    )
...
async def eval_rollout_single_dataset(
    ...
    if cache_key not in EVAL_PROMPT_DATASET:
        ...
        EVAL_PROMPT_DATASET[cache_key] = Dataset(
            ...
            source_key=args.eval_source_key if args.eval_source_key else args.source_key,  # LNY: Extended keys for source_config support
    ...
    for _i, prompt_sample in enumerate(dataset.samples):
        for j in range(dataset_cfg.n_samples_per_eval_prompt):
            ...
            sample.metadata["session_id"] = "session-" + str(uuid.uuid4())
            sample_index += 1
    ...
    eval_results = {
        ...
        "response_length": [sample.response_length for sample in data],
        "truncated": [sample.status == Sample.Status.TRUNCATED for sample in data],
        "prompt": [sample.prompt for sample in data],
        "response": [sample.response for sample in data],
    ...
    for extra_key in [
        ...
        "ill_formed",
        "history",
        "outcome_reward",  # LNY: passed to downstream logging
        "rubric_reward",
    ]:
        if data[0].metadata and extra_key in data[0].metadata:
            eval_results[extra_key] = [sample.metadata[extra_key] for sample in data]

    return {
        dataset_cfg.name: eval_results,  # 'aa-lcr': {'rewards': [0], 'response_length': [2736], ...}
    }
