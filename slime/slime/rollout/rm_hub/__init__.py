...
async def remote_rm(args, sample: Sample, max_retries: int = 10, **kwargs):
    assert "remote_url" in kwargs, "remote_url is required for remote RM"
    ...
    if input_keys:
        ...
        for key in input_keys:
            if hasattr(sample, key):
                payload[key] = getattr(sample, key)  # sample is not a dict, cannot use sample[key]; use sample.key instead
            elif key in sample.metadata:
                payload[key] = sample.metadata[key]
            else:
                # print(f"sample has no key '{key}'")
...
async def async_rm(args, sample: Sample, **kwargs):
    ...
    if "reward_model_info" in metadata:
        if not args.no_punish_for_illform:
            is_ill_formed = metadata["ill_formed"]
            is_task_unfinished = metadata["task_unfinished"]
            is_evaluation = metadata["is_evaluation"]
            if (sample.status != Sample.Status.COMPLETED or is_ill_formed or is_task_unfinished) and not is_evaluation:
                print(f"Reward = 0: status: {sample.status}, is_ill_formed: {is_ill_formed}, is_task_unfinished: {is_task_unfinished}, is_evaluation: {is_evaluation}")
                return {
                    "reward": 0,
                    "outcome_reward": 0,
                    "rubric_reward": 0,
        ...
        rm_kwargs = reward_model_info.get("kwargs", {})
        if rm_function_name == "remote_rm":
            return await remote_rm(args, sample, **rm_kwargs)
        else:
            rm_function = load_function(rm_function_name)
            return await rm_function(sample, **rm_kwargs)
