...
def _log_eval_rollout_data(rollout_id, args, data, extra_metrics: dict[str, Any] | None = None):
    ...
    for key in data.keys():  # LNY: key is the eval dataset name, e.g. aa-lcr
        ...
        # LNY: Log extra per-sample metrics
        for metric in [
            "response_length",
            "use_tool",
            "env_interact_steps",
            ...
            if metric in data[key]:
                ...
                log_dict[f"eval/{key}-{metric}"] = sum(scores) / len(scores)  # LNY: average each metric across all eval samples. E.g. 100 prompts with n_samples_per_eval_prompt=3 yields 300 scores, averaged here.
    ...
    # LNY: Save detailed eval results if configured
    if args.eval_results_save_dir is not None:
        import json

        save_dir = f"{args.eval_results_save_dir}/step-{step}"
        os.makedirs(save_dir, exist_ok=True)
        json.dump(log_dict, open(f"{save_dir}/overall.json", "w"), indent=2, ensure_ascii=False)
        for key in data.keys():
            results = []
            for i in range(len(data[key]["rewards"])):
                result = {}
                for term in [
                    "prompt",
                    "response",
                    ...
                    if term in data[key]:
                        result[term] = data[key][term][i]
                results.append(result)
            json.dump(results, open(f"{save_dir}/results-{key}.json", "w"), indent=2, ensure_ascii=False)

    return log_dict


def _log_per_source_metrics(log_dict, args, samples):
    """Report group reward distribution and per-source metric averages."""
    # Collect per-sample values
    per_sample_data = {
        "sources": [sample.source for sample in samples],
        # "tokens": [sample.tokens for sample in samples],
        "response_lengths": [sample.response_length for sample in samples],
        "raw_reward": [sample.reward if not isinstance(sample.reward, dict) else sample.get_reward_value(args) for sample in samples],
        "truncated": [1 if sample.status == Sample.Status.TRUNCATED else 0 for sample in samples],
        # "samples_count": len(samples),
        "unmasked_sample_ratio": [(sample.loss_mask is None or sum(sample.loss_mask) > 0) for sample in samples],
    }
    for extra_key in [
        "loss_token_num",
        "use_tool",
        ...
        if samples[0].metadata and extra_key in samples[0].metadata:
            per_sample_data[extra_key] = [sample.metadata.get(extra_key, 0) for sample in samples]

    # Count reward distribution by group
    rewards = per_sample_data["raw_reward"]
    if len(rewards) > 0 and hasattr(args, "n_samples_per_prompt"):
        ...
        for i, reward in enumerate(rewards):
            group_index = i // n
            group_rewards.setdefault(group_index, []).append(reward)
        
        # Count different group types
        total_groups = len(group_rewards)
        all_one_groups = 0  # Groups where all rewards are 1
        non_all_zero_groups = 0  # Groups with mixed rewards
        all_zero_groups = 0  # Groups where all rewards are 0
        for group_index, group_reward_list in group_rewards.items():
            if all(r == 1.0 for r in group_reward_list):
                all_one_groups += 1
            elif all(r == 0.0 for r in group_reward_list):
                all_zero_groups += 1
            else:
                non_all_zero_groups += 1
        if total_groups > 0:
            log_dict["rollout/group_all_one_ratio"] = all_one_groups / total_groups
            log_dict["rollout/group_non_all_zero_ratio"] = non_all_zero_groups / total_groups
            log_dict["rollout/group_all_zero_ratio"] = all_zero_groups / total_groups

    # Per-source averages
    sources = per_sample_data["sources"]
    split_keys = [
        "response_lengths",
        "raw_reward",
        "truncated",
        "loss_token_num",
        "unmasked_sample_ratio",
        "use_tool",
        "env_interact_steps",
        "task_unfinished",
        "ill_formed",
    ...
    if any(s is not None for s in sources):
        ...
        for idx, src in enumerate(sources):
            if src is not None:
                source2idxs.setdefault(src, []).append(idx)
        for key in split_keys:
            if key not in per_sample_data:
                continue
            val = per_sample_data[key]
            if not val or not isinstance(val[0], (int, float, bool)):
                continue
            for src, idxs in source2idxs.items():
                val_src = [val[i] for i in idxs]
                log_dict[f"rollout/{src}/{key}"] = sum(float(v) for v in val_src) / len(val_src)
...
def _log_rollout_data(rollout_id, args, samples, rollout_extra_metrics, rollout_time):
    ...
    # Extra metadata metrics (per-source aggregation)
    _log_per_source_metrics(log_dict, args, samples)
